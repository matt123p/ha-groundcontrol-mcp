...
def test_root_hacs_manifest_exists() -> None:
    root_manifest = Path(__file__).parents[1] / "manifest.json"
    assert root_manifest.exists(), "Root HACS manifest.json is missing"

    payload = json.loads(root_manifest.read_text(encoding="utf-8"))
    assert payload["name"] == "HA GroundControl MCP"
    assert payload["content_in_root"] is True
